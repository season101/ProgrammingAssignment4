...
# ###################### Introduction #############################
#                                                                 #
#    #Name: Sijan Malla              Date Assigned: Oct. 10,2019  #
#    #                                                            #
#    #Course: 2000-44306             Date Due : Oct. 24, 2019     #
#    #                                                            #
#    #File name: Programming Assignment 4                         #
#    #                                                            #
#    #Program Description: It is a program that simply Decodes    #
...
while userChoice == "y":        #Looping for if user wants to run the program again. For this user must input userChoice as "y".
    ...
    while fileName[len(fileName)-4: ]!=".txt":      #input validation for ensuring that the given file must be a .txt format
    ...
    for everySentence in fileToDecode:      #For loop to run until there is sentence in a given file. It reads each lines in single loop.
        LineInFile = everySentence     #LineInFile is used to store each lines one at a time so as to pass its value to inner for everyCharacter Loop.
        ...
        for everyCharacter in LineInFile:       #For loop to run until there is character in a given line. It reads each charcter in single loop.
            ...
            if ord(everyCharacter)>=33:     #To check if given character is space i.e. word seperator. Since, we don't want to use any other character as word seperator. If true ie. given character is not " " it executes.
                ...
                decodedSentence+= chr(ASCIIForDecodedCharacter)        #Accumulating character one after another in its decoded form

            elif ord(everyCharacter)==31:
                decodedSentence+="!"       #It is to ignore the fact that we want exclamation sign to be there.
        ...
        decodedSentence=""     #Resetting the value for sentence to zero so that when new line is read from outer loop the values for earlier sentence in deleted from the variable "sentence"
    ...
    userChoice = input("Would you like to decode another file(y/n)?: ")     ##asking user if they want to run program again.
    userChoice = userChoice.lower()

    while not(userChoice == "y" or userChoice == "n"):      #input validation for userchoice
        userChoice = input("Error! Enter your choice(y and n are only allowed): ")

print()

print("Thank you for using our program. Please come again!") #Ending Prompt
